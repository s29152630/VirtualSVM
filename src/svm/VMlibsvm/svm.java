...
public class svm {
	...
	private static void svm_group_classes(VirtualMatrix prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, VirtualMatrix perm, List<Integer> y)
	{
		int l = prob.nSize();
		int max_nr_class = 16;
		int nr_class = 0;
		...
		List<Integer> data_label = new ArrayList<Integer>();
		int i;

		for(i=0;i<l;i++)
		{
			int this_label = (int)(y.get(i));
			int j;
		...
		for(i=0;i<l;i++)
		{
			perm.alter(i, 0, start[data_label.get(i)]);
			++start[data_label.get(i)];
		}
		start[0] = 0;
		for(i=1;i<nr_class;i++)
			start[i] = start[i-1]+count[i-1];
	...
	// Interface functions
	//
	public static svm_model svm_train(VirtualMatrix vM, svm_parameter param, List<Integer> y)
	{
		...
		model.param = param;
			...
			VirtualMatrix perm = new VirtualMatrix(); // how to set ?x? ; start 0 or 1?

			// group training data of the same class
			svm_group_classes(vM,tmp_nr_class,tmp_label,tmp_start,tmp_count,perm, y);
			...
			// 把 vM 依照 label 整理好，將 vM 複製到 x
			VirtualMatrix x = new VirtualMatrix(); // how to set ?x?

			int i;
			for(i=1;i<=l;i++){
				List<Double> vMRow = (List<Double>) vM.getRow(perm.getColumnList(i).indexOf(0));
				for(int j=1;;j++){
					x.alter(vMRow.get(j), i, j);
				}
			}
			...
			VirtualMatrix nonzero = new VirtualMatrix(); // 一維的 VM 長度為 l, initial value = false;
